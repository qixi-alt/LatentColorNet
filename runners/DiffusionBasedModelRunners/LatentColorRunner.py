...
from model.BrownianBridge.LatentColorModel import LatentColorModel
...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def initialize_model(self, config):
        if config.model.model_type == "BBDM":
            bbdmnet = BaseModel(config.model).to(config.training.device[0])
        elif config.model.model_type == "LBBDM":
            bbdmnet = LatentColorModel(config.model).to(config.training.device[0])
        else:
